...
export default class Game extends BaseGame {
	...
	generateWorld(options) {
		...
		if (!options.id) {
			throw new Error('World definition must have an "id" property');
		}

		if (!options.layers || !Array.isArray(options.layers) || options.layers.length === 0) {
			throw new Error(`World "${options.id}" must have at least one layer defined`);
		}

		options.layers.forEach((layer, index) => {
			if (!layer.ground) {
				throw new Error(`Layer ${index} in world "${options.id}" missing required 'ground' property`);
			}

			['holeChance', 'mineralChance', 'hazardChance', 'itemChance'].forEach(prop => {
				if (layer[prop] !== undefined && (layer[prop] < 0 || layer[prop] > 100)) {
					throw new Error(`Layer ${index} ${prop} must be between 0-100, got ${layer[prop]}`);
				}
			});
		});
		...
		const world = {
			...
			width: [30, 50],
			depth: [120, 180],
			transportChoices: Object.keys(WORLDS),
			tunnelSystems: {
				count: [2, 5], // Number of tunnel systems to generate
				branchProbability: 0.3, // Chance each segment branches
				maxBranches: 3, // Maximum branches per system
				segmentLength: [3, 8], // Length of each tunnel segment
				wanderStrength: 0.4, // How much tunnels can curve (0-1)
			},
			newPlayer: {
				credits: 0,
				items: {},
				...
				configuration: {
					...
					drill: 'T1',
					engine: 'T1',
					...options.newPlayer?.configuration,
				},
			},
			...options,
			grid: [],
		...
		// Process array values and validate ranges
		['airGap', 'safeDepth', 'width', 'depth'].forEach(key => {
			if (Array.isArray(world[key])) {
				if (world[key].length !== 2 || world[key][0] > world[key][1]) {
					throw new Error(`Invalid range for ${key} in world "${world.name}": must be [min, max] where min <= max`);
				}
				if (world[key][0] < 0) {
					throw new Error(`Invalid range for ${key} in world "${world.name}": values must be non-negative`);
				}
				world[key] = randInt(...world[key]);
			}

			if (typeof world[key] !== 'number' || world[key] < 0) {
				throw new Error(`${key} must be a non-negative number, got ${world[key]}`);
			}
		});

		if (world.width < 6) {
			throw new Error(`World width too small (${world.width}) to place SpaceCo outpost`);
		}
		if (world.depth < 10) {
			throw new Error(`World depth too small (${world.depth}) for viable gameplay`);
		}
		if (world.airGap >= world.depth) {
			throw new Error(`Air gap (${world.airGap}) must be less than world depth (${world.depth})`);
		}
		if (world.safeDepth >= world.depth) {
			throw new Error(`Safe depth (${world.safeDepth}) must be less than world depth (${world.depth})`);
		}

		// Process tunnel system configuration
		if (world.tunnelSystems) {
			['count', 'segmentLength'].forEach(key => {
				if (Array.isArray(world.tunnelSystems[key])) {
					if (world.tunnelSystems[key].length !== 2 || world.tunnelSystems[key][0] > world.tunnelSystems[key][1]) {
						throw new Error(`Invalid tunnelSystems.${key} range: must be [min, max] where min <= max`);
					}
					world.tunnelSystems[key] = randInt(...world.tunnelSystems[key]);
				}
			});

			['branchProbability', 'wanderStrength'].forEach(key => {
				if (world.tunnelSystems[key] !== undefined && (world.tunnelSystems[key] < 0 || world.tunnelSystems[key] > 1)) {
					throw new Error(`tunnelSystems.${key} must be between 0-1, got ${world.tunnelSystems[key]}`);
				}
			});
		}

		// Process SpaceCo configuration
		['variant', 'health'].forEach(key => {
			if (Array.isArray(world.spaceco[key])) {
				if (world.spaceco[key].length !== 2 || world.spaceco[key][0] > world.spaceco[key][1]) {
					throw new Error(
						`Invalid range for spaceco.${key} in world "${world.name}": must be [min, max] where min <= max`,
					);
				}
				world.spaceco[key] = randInt(...world.spaceco[key]);
			}
		});

		// Process SpaceCo equipment lists
		['vehicles', 'drills', 'engines', 'parts'].forEach(key => {
			const allItems = { vehicles, drills, engines, parts }[key];
			const itemNames = Object.keys(allItems);

			if (world.spaceco[key] === 'random') {
				if (itemNames.length === 0) {
					throw new Error(`No ${key} definitions found for random selection`);
				}
				// Create a proper copy and shuffle it
				const shuffledNames = [...itemNames];
				shuffleArray(shuffledNames);
				world.spaceco[key] = shuffledNames
					.slice(0, randInt(3, Math.min(itemNames.length, 8)))
					.filter(name => allItems[name]); // Double-check validity
			} else if (Array.isArray(world.spaceco[key])) {
				const invalidItems = world.spaceco[key].filter(name => !allItems[name]);
				if (invalidItems.length > 0) {
					throw new Error(`Invalid ${key} specified: ${invalidItems.join(', ')}`);
				}
			}
		});

		// Process shop configuration
		...
		if (!world.spaceco.originalShop) {
			world.spaceco.originalShop = { ...world.spaceco.shop };
		}

		Object.keys(world.spaceco.shop).forEach(itemName => {
			if (!validShopItems.includes(itemName)) {
				throw new Error(`Invalid shop item "${itemName}" in world "${world.name}"`);
			}

			if (Array.isArray(world.spaceco.shop[itemName])) {
				if (world.spaceco.shop[itemName].length !== 2) {
					throw new Error(`Invalid shop range for ${itemName}: must be [min, max]`);
				}
				const [min, max] = world.spaceco.shop[itemName];
				if (min < 0 || max < min) {
					throw new Error(`Invalid shop range for ${itemName}: min must be >= 0, max must be >= min`);
				}
				world.spaceco.shop[itemName] = randInt(min, max);
				world.spaceco.originalShop[itemName] = world.spaceco.shop[itemName];
			} else {
				// Store the original base value for later scaling
				world.spaceco.originalShop[itemName] = world.spaceco.shop[itemName];
			}
		});

		// Set up transport system
		world.transports = Object.fromEntries(
			worlds.flatMap(({ name, transportPrice, transportRequirements }) => {
				if (!world.transportChoices.includes(name)) return [];
				return [[name, { price: transportPrice, requirements: transportRequirements }]];
			}),
		);

		// Set SpaceCo position
		if (!world.spaceco.position) {
			world.spaceco.position = { x: randInt(3, world.width - 3), y: world.airGap };
		} else {
			if (
				world.spaceco.position.x < 3 ||
				world.spaceco.position.x >= world.width - 3 ||
				world.spaceco.position.y < 0 ||
				world.spaceco.position.y >= world.depth
			) {
				throw new Error(`SpaceCo position (${world.spaceco.position.x}, ${world.spaceco.position.y}) is out of bounds`);
			}
		}

		const validateWeightedDistribution = (distribution, context) => {
			...
			if (keys.length === 0) {
				throw new Error(`Empty weighted distribution in ${context}`);
			}

			if (values.some(v => typeof v !== 'number' || v < 0)) {
				throw new Error(`Weighted distribution in ${context} must have non-negative numeric values`);
			}

			const sum = values.reduce((acc, val) => acc + val, 0);
			if (Math.abs(sum - 100) > 0.1) {
				throw new Error(`Weighted distribution in ${context} must sum to 100, got ${sum.toFixed(1)}`);
			}
		};

		const resolveDistribution = (value, validValues, context) => {
			if (typeof value === 'string') {
				if (value === 'random') {
					if (validValues.length === 0) {
						throw new Error(`Cannot select random ${context}: no valid values available`);
					}
					return randFromArray(validValues);
				}
				if (value === 'current') {
					return 'current'; // Special case for minerals
				}
				if (!validValues.includes(value)) {
					throw new Error(`Invalid ${context} value "${value}". Must be one of: ${validValues.join(', ')}`);
				}
				return value;
			}
			...
			if (Array.isArray(value)) {
				if (value.length === 0) {
					throw new Error(`Empty array for ${context}`);
				}
				const invalidValues = value.filter(v => !validValues.includes(v));
				if (invalidValues.length > 0) {
					throw new Error(
						`Invalid ${context} values: ${invalidValues.join(', ')}. Must be from: ${validValues.join(', ')}`,
					);
				}
				return randFromArray(value);
			}

			if (typeof value === 'object' && value !== null) {
				validateWeightedDistribution(value, context);
				const invalidKeys = Object.keys(value).filter(k => !validValues.includes(k));
				if (invalidKeys.length > 0) {
					throw new Error(
						`Invalid ${context} keys: ${invalidKeys.join(', ')}. Must be from: ${validValues.join(', ')}`,
					);
				}
				try {
					return weightedChance(value);
				} catch (error) {
					throw new Error(`Error resolving weighted ${context}: ${error.message}`);
				}
			}

			throw new Error(`Invalid ${context} type: ${typeof value}. Must be string, array, or weighted object`);
		};

		const layerBoundaries = [];
		const usableDepth = world.depth - world.airGap - 1; // Exclude air gap and surface
		const layerDepth = Math.floor(usableDepth / world.layers.length);
		const extraDepth = usableDepth % world.layers.length; // Handle remainder

		for (let i = 0; i < world.layers.length; i++) {
			const startY = world.airGap + 1 + i * layerDepth;
			let endY = startY + layerDepth - 1;

			// Distribute extra depth to last layers
			if (i >= world.layers.length - extraDepth) {
				endY += 1;
			}

			// Ensure last layer goes to world bottom
		...
		const generateTunnelSystems = () => {
			const tunnelMap = new Set(); // Track tunnel positions as "x,y" strings

			const createTunnelSystem = () => {
				// Start tunnel system at a random position below safe depth
				const startX = randInt(1, world.width - 2);
				const startY = randInt(Math.max(world.safeDepth, world.airGap + 2), world.depth - 2);

				const tunnelPositions = [];
				const branches = [{ x: startX, y: startY, direction: null, remaining: world.tunnelSystems.segmentLength }];

				while (branches.length > 0) {
					const branch = branches.shift();

					for (let step = 0; step < branch.remaining; step++) {
						const posKey = `${branch.x},${branch.y}`;

						if (
							branch.x < 1 ||
							branch.x >= world.width - 1 ||
							branch.y <= world.airGap ||
							branch.y >= world.depth - 1
						) {
							break;
						}

						if (!tunnelMap.has(posKey)) {
							tunnelMap.add(posKey);
							tunnelPositions.push({ x: branch.x, y: branch.y });
						}

						if (!branch.direction) {
							// First step - choose initial direction (prefer horizontal or slight down)
							const directions = ['horizontal', 'down', 'diagonal'];
							branch.direction = randFromArray(directions);
						}

						if (chance(world.tunnelSystems.wanderStrength * 100)) {
							const directions = ['horizontal', 'down', 'diagonal'];
							branch.direction = randFromArray(directions);
						}
						...
						const moves = moveOptions[branch.direction];
						const move = randFromArray(moves);
						branch.x += move.dx;
						branch.y += move.dy;

						if (
							step > 2 &&
							branches.length < world.tunnelSystems.maxBranches &&
							chance(world.tunnelSystems.branchProbability * 100)
						) {
							const branchDirections = Object.keys(moveOptions).filter(d => d !== branch.direction);
							const branchDirection = randFromArray(branchDirections);
							const branchLength = randInt(
								...(Array.isArray(world.tunnelSystems.segmentLength)
									? world.tunnelSystems.segmentLength
									: [3, world.tunnelSystems.segmentLength]),
							);
			...
			const allTunnels = [];
			for (let i = 0; i < world.tunnelSystems.count; i++) {
				const system = createTunnelSystem();
				allTunnels.push(...system);
			}
		...
		try {
			for (let x = 0; x < world.width; x++) {
				...
				for (let y = 0; y < world.depth; y++) {
					...
					// Find layer for this position
					const layerInfo = layerBoundaries.find(({ start, end }) => y >= start && y <= end);
					if (!layerInfo) {
						throw new Error(`No layer found for position (${x}, ${y}) - this should not happen`);
					}
					...
					let rockType;
					try {
						rockType = resolveDistribution(
							layer.ground,
							mineralColors,
							`ground in layer ${layerIndex} at (${x}, ${y})`,
						);
					} catch (error) {
						throw new Error(`Error in layer ${layerIndex} at position (${x}, ${y}): ${error.message}`);
					}

					// Determine if this cell becomes solid rock or open space
					const isPreGeneratedTunnel = preGeneratedTunnels.has(`${x},${y}`);
					const shouldBeHole = y >= world.safeDepth && (isPreGeneratedTunnel || chance(holeChance));
					// const isBottomLayer = layerIndex === world.layers.length - 1;

					if (!shouldBeHole) {
						// Solid rock
						world.grid[x][y].ground = {};
						world.grid[x][y].ground.type = rockType;

						// Add minerals to rock
						if (chance(mineralChance)) {
							let mineralType;
							try {
								if (mineralsDef === 'current') {
									mineralType = rockType;
								} else {
									mineralType = resolveDistribution(
										mineralsDef,
										mineralColors,
										`minerals in layer ${layerIndex} at (${x}, ${y})`,
									);
								}
							} catch (error) {
								throw new Error(`Error resolving minerals in layer ${layerIndex} at (${x}, ${y}): ${error.message}`);
							}

							if (!minerals[mineralType]) {
								throw new Error(`Unknown mineral type "${mineralType}" generated at (${x}, ${y})`);
							}

							const baseMineralCount = randInt(1, chance(25) ? 2 : 4);
							const depthBonus = depthPercent > 0.9 ? randInt(0, 1) : 0;
							const mineralCount = baseMineralCount + depthBonus;

							world.grid[x][y].items = [];

							for (let i = 0; i < mineralCount; i++) {
								world.grid[x][y].items.push({ name: `mineral_${mineralType}` });
							}
						}
					} else {
						...
						if (!contentAdded && hazardDef && hazardChance > 0 && chance(hazardChance)) {
							try {
								const hazard = this.resolveHazard(hazardDef, `layer ${layerIndex} at (${x}, ${y})`);

								// Handle mystery spawners
								if (hazard.type === 'mystery_spawner') {
									// Resolve what the mystery spawner actually creates
									const spawnChoice = weightedChance(
										Object.fromEntries(hazard.spawnTable.map(entry => [JSON.stringify(entry), entry.weight])),
									);
									const chosenEntry = JSON.parse(spawnChoice);
									const spawn = chosenEntry.spawn;

									// Check for elite variants
									if (spawn.eliteChance && chance(spawn.eliteChance)) {
										// Try to find elite variant
										const eliteVariants = {
											psykick_warrior: 'elite_psykick_warrior',
											depth_guardian: 'ancient_depth_guardian',
											tunnel_chomper: 'alpha_tunnel_chomper',
											lava_spitter: 'volatile_lava_spitter',
											gas_sporecyst: 'enhanced_gas_sporecyst',
											spawn_mother: 'grand_spawn_mother',
											gift_bearer: 'master_gift_bearer',
										};

										if (spawn.type === 'alien' && eliteVariants[spawn.name]) {
											spawn.name = eliteVariants[spawn.name];
										}
									}

									world.grid[x][y].hazards = [];

									// Create the actual spawned content
									switch (spawn.type) {
										case 'alien':
											world.grid[x][y].hazards.push({
												type: 'alien',
												name: spawn.name,
												orientation: 'right',
											});
											break;
										case 'item':
											world.grid[x][y].items.push({ name: spawn.name });
											break;
										case 'hazard':
											world.grid[x][y].hazards.push({ type: spawn.name });
											break;
										case 'ground':
											// Convert open space to solid ground
											world.grid[x][y].ground = { type: spawn.name };
											break;
									}
								} else {
									world.grid[x][y].hazards.push(hazard);
								}
								contentAdded = true;
							} catch (error) {
								throw new Error(`Error creating hazard in layer ${layerIndex} at (${x}, ${y}): ${error.message}`);
							}
						}

						// 2. Add regular hazards
						if (!contentAdded && hazardDef && hazardChance > 0 && chance(hazardChance)) {
							try {
								world.grid[x][y].hazards = world.grid[x][y].hazards || [];
						...
						// 3. Add items if no hazards were added
						if (!contentAdded && itemDef && chance(itemChance)) {
							try {
								const itemName = resolveDistribution(
									itemDef,
									itemNames,
									`items in layer ${layerIndex} at (${x}, ${y})`,
								);

								if (!items[itemName]) {
									throw new Error(`Unknown item type "${itemName}" generated at (${x}, ${y})`);
								}

								world.grid[x][y].items = world.grid[x][y].items || [];

								world.grid[x][y].items.push({ name: itemName });
								contentAdded = true;
							} catch (error) {
								throw new Error(`Error creating item in layer ${layerIndex} at (${x}, ${y}): ${error.message}`);
							}
						}
					}
				}
			...
			// Post-process: Improved hazard spreading after all content is placed
			this.postProcessHazardSpreading(world);
		} catch (error) {
			worldLog.error(`World generation failed`, {
		...
		// Final validation
		this.validateGeneratedWorld(world);
